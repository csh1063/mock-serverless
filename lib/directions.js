    ...
    const routes = data.routes.map((route) => {
        ...
        const steps = (leg.steps || []).map((step) => {
            ...
            if (step.travel_mode === 'TRANSIT' && step.transit_details) {
                ...
                // 노선 고유 브랜드 색 — GTFS 피드에 route_color가 있는 지역/노선만 내려온다
                // (없으면 undefined인 채로 두고, 클라이언트가 이동수단별 기본 색으로 대체).
                if (step.transit_details.line.color) {
                    s.lineColor = step.transit_details.line.color;
                }
                if (step.transit_details.line.text_color) {
                    s.lineTextColor = step.transit_details.line.text_color;
                }
            }
            return s;
        });
